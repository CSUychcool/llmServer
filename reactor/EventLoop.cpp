...
int EventLoop::add(Channel* channel)
{
    ...
    // 诊断: fd 已在 channelMap 中(复用后未清除 -> 新连接注册失败 -> 僵死)
    fprintf(stderr, "[EventLoop] WARN: add() rejected, fd=%d already in channelMap (stale entry!)\n", fd);
    return -1;
}
...
int EventLoop::remove(Channel* channel)
{
    ...
    m_dispatcher->setChannel(channel);
    int ret = m_dispatcher->remove();    // 内部触发 destroyCallback: close(fd) + delete conn
    // 关键: 从 channelMap 摘除并销毁 channel, 否则 fd 复用后 add() 会拒绝注册 -> 新连接僵死
