    ...
    if (end != nullptr)
    {
        ...
        if (middle != nullptr)
        {
            ...
            if (keyLen > 0 && valueLen > 0)
            {
                ...
                addHeader(key, value);
                // 补充：识别 Content-Length 头，供 ParseReqBody 使用
                if (strcasecmp(key.data(), "content-length") == 0)
                {
                    curContentLength = atoi(value.c_str());
                }
            }
            // 移动读数据的位置
            readBuf->readPosIncrease(lineSize + 2);
...
{
    if (curContentLength > 0 && readBuf->readableSize() >= curContentLength)
    {
        // 从 buffer 中读取完整 body，同时推进读指针
        m_body = readBuf->data(curContentLength);
        curContentLength = 0;
        setState(PrecessState::ParseReqDone);
        return true;
    }
    return false; // body 还没收全，等下一次 read
}
...
{
    // ---- 补充：支持自定义路由钩子（POST /api/chat 等） ----
    if (m_router)
    {
        m_router(m_body, response);
        return true;
    }

    // ---- 原有纯 GET 静态文件逻辑（保持向后兼容） ----
